...
\begin{document}
    ...
    图 \ref{fig:result} 显示了运行结果的损耗统计图，根据上面的运行结果给出（请注意同族算法有多种实现，结果只给出其中一种）。可见动态规划和 A* 都给出了最优解，而遗传算法只能得到一个次优解，差距%
    ...
    \begin{figure}[h]
        ...
        \begin{tikzpicture}
            ...
            symbolic x coords={dp,mdp,ndp,astar,hastar,ga,sga}
             ...
             \addplot+ [] coordinates { (ga,72) (sga,60)};
            \end{axis}
            \end{tikzpicture}            
    \end{figure}

    图 \ref{fig:time} 显示了各种实现的运行时间。可见，在二维情景下，dp 是最快的；在三维场景下，hastar（根据二维dp改进的A*）是最快的。现有的遗传算法实现在二维和三维中没有发挥其优势，主要受制于限制时间，其优势主要在高维时可以任意限制搜索时间，而不像传统的动态规划和 A* 算法需要等待结果返回。
