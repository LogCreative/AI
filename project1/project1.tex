...
\begin{document}
    ...
    可以统一化为多序列比对问题。对于 $L$ 条序列比对，其行动转换方法可以被表示为二进制从 $(\underbrace{0\cdots01}_{L\text{ digits}})_2$ 到 $(\underbrace{1\cdots11}_{L\text{ digits}})_2$ 内所有的数（最低位为第一维度），计算损耗使用上三角成对比较，规则统一为
    \begin{equation*}
        \texttt{compare}=
        \begin{cases}
            0,& (-,-) \| (p,p) \\
    ...
    \begin{algorithm}[h]
        ...
        \KwOut{minimum cost}
        \BlankLine
        $openSet\leftarrow{origin}$\;
        $dist$
        % \Repeat{\texttt{pos}$=$finish}{
        %     $minmove\leftarrow$NULL\;
        %     $mineval\leftarrow\infty$\;
        %     \ForEach{avaliable move}{
        %         $n\leftarrow$\texttt{pos}$+$\emph{avaliable move}\;
        %         $f(n)=cost+\texttt{comparelist}(\textit{avaliable move})+h(n)$\;
        %         \If{$f(n)<mineval$}{
        %             $minmove\leftarrow$\emph{avaliable move}\;
        %             $mineval\leftarrow f(n)$\;
        %         }
        %     }
        %     \texttt{pos}$\leftarrow$\texttt{pos}$+$\emph{minmove}\;
        %     $cost\leftarrow cost+\texttt{comparelist}(\textit{minmove})$\;
        % }
        \Return{cost}\;
    \end{algorithm}
    ...
    % 该算法的时间复杂度，对于 $L$ 个字符串（平均长度为 $l$），最多进行 $\sum_i l_i$ 步，每一步的分支因子为 $2^{L}-1$，单个实例需要花费时间
    % \begin{equation*}
